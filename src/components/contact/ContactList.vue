...
import { alertError, alertConfirm } from '../../lib/alert';
...
async function handleDelete(id) {
	if (!await alertConfirm('Are you sure you want to delete this contact?')) return;
...
<template>
	...
	<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
		...
		<div
			...
			<RouterLink to="contacts/create" class="block p-6 h-full">
			...
			<div class="p-6">
				<RouterLink :to="`/dashboard/contacts/${contact.id}/edit`"
					...
					<div class="flex items-center mb-3">
						...
						<h2 class="text-xl font-semibold text-white hover:text-blue-300 transition-colors duration-200">{{ contact.first_name }} {{ contact.last_name }}
						</h2>
					</div>
	...
	<div class="mt-10 flex justify-center">
		<nav
			...
			<a href="#" v-if="page > 1" @click="handleChangePage(page - 1)"
			...
			<a href="#" v-for="value in pages" :key="value" @click="handleChangePage(value)"
				:class="[
					page === value ? 'px-4 py-2 bg-gradient text-white rounded-lg hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-800 transition-all duration-200 font-medium shadow-md' : 'px-4 py-2 bg-gray-700 text-gray-300 rounded-lg hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-800 transition-all duration-200'
				]">
				{{ value }}
			</a>
			<!-- <a href="#"
			...
			<a href="#" v-if="page < totalPage" @click="handleChangePage(page + 1)"
				class="px-4 py-2 bg-gray-700 text-gray-300 rounded-lg hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-800 transition-all duration-200 flex items-center">
